...
// تم تصحيح مراجع proto هنا لتستخدم WAProto_1.proto
const getUserAgent = (config) => {
    const osName = (config.browser?.[0] || '').toLowerCase();

    const platform =
        osName === 'and_roid'
            ? WAProto_1.proto.ClientPayload.UserAgent.Platform.SMB_ANDROID
            : osName === 'android'
    ...
    return {
        ...
        device: osName.includes('android') ? 'Android' : osName.includes('and_roid') ? 'Android' : 'Desktop',
        osBuildNumber: config.browser?.[2] || '0.1',
...
const getPlatformType = (platform) => {
    ...
    if (platformType === 'ANDROID' || platformType === 'AND_ROID') {
        return WAProto_1.proto.DeviceProps.PlatformType.ANDROID_PHONE;
    } else {
        // التحقق من وجود القيمة في enum لتجنب الأخطاء
