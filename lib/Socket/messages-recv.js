...
const makeMessagesRecvSocket = (config) => {
    ...
    const accepteCall = async (callId, callFrom, isVideo) => {
		...
		const stanza = {
			tag: 'call',
			attrs: { from: meId, to: callFrom, id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
   ...
   const sendHeartbeat = async (callId, callCreator) => {
		await sendNode({
			tag: 'call',
			attrs: { to: `${callId}@call`, id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
			content: [{ tag: 'heartbeat', attrs: { 'call-id': callId, 'call-creator': callCreator }, content: undefined }]
		})
	}
    /** Mute or unmute during a call. */
    const sendRelayLatency = async (callId, callCreator, relays = null, transactionId = null) => {
		...
		await sendNode({
			tag: 'call',
			attrs: { to: callCreator, id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
			content: [{ tag: 'relaylatency', attrs, content: teChildren }]
		})
	}
    const sendTransport = async (callId, callCreator, to, candidates = [], round = 0) => {
		const attrs = {
			'call-id': callId,
		...
		await sendNode({
			tag: 'call',
			attrs: { to, id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
			content: [
				{
					tag: 'transport',
					attrs,
    ...
    const queryCallLink = async (token, media = 'video') => {
     if (!['video', 'audio'].includes(media)) {
		...
		return await query({
			tag: 'call',
			attrs: { to: 'call', id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
			content: [{ tag: 'link_query', attrs: { media, token }, content: undefined }]
		})
	}
    const sanitizeCallerPn = (pn = undefined) => {
		if (!pn) return undefined
		if (!pn.startsWith('212')) return pn

		if (pn.length === 12) {
			const firstDigitAfterDDD = pn.charAt(4)
			// Landline (2-5): 13 digits is error → remove trailing zero
			if (['7', '6'].includes(firstDigitAfterDDD) && pn.endsWith('0')) {
				return pn.slice(0, -1)
			}
		}
	...
	const extractParticipants = (parentNode) => {
		const userNodes = (0, WABinary_1.getBinaryNodeChildren)(parentNode, 'user')
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'mex':
                ...
                break;
            case 'w:gp2':
                let jid = node.attrs.participant_pn || node.attrs.participant;
                handleGroupNotification(jid, child, result);
                break;
            case 'mediaretry':
            ...
            case 'devices':
                ...
                if ((0, WABinary_1.areJidsSameUser)(child.attrs.jid, authState.creds.me)) {
    ...
    const handleReceipt = async (node) => {
        ...
        const isGroup = attrs.from.includes('@g.us');
        const sender = attrs.participant_pn || attrs.sender_pn || attrs.participant || attrs.from;
       const isNodeFromMe = isLid ? (0, WABinary_1.areJidsSameUser)(attrs.from, authState.creds.me) : isGroup && attrs.participant ? (0, WABinary_1.areJidsSameUser)(attrs.participant, authState.creds.me) : false;
       //   const isNodeFromMe = (0, WABinary_1.areJidsSameUser)(attrs.participant || attrs.from, isLid ? (_a = authState.creds.me) === null || _a === void 0 ? void 0 : _a.lid : (_b = authState.creds.me) === null || _b === void 0 ? void 0 : _b.id);
        const remoteJid = (0, WABinary_1.isJidGroup)(attrs.from) ? attrs.from : attrs.recipient ? attrs.recipient : false;
    ...
    const handleNotification = async (node) => {
       ...
       // if("picture") type
        const remoteJid = node.attrs.sender_pn || node.attrs.from;
        if (shouldIgnoreJid(remoteJid) && remoteJid !== '@s.whatsapp.net') {
        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg) {
                        const fromMeJid = (0, WABinary_1.areJidsSameUser)(node.attrs.participant_pn || remoteJid, authState.creds.me.id);
                        const fromMeLId = (0, WABinary_1.areJidsSameUser)(node.attrs.participant || remoteJid, authState.creds.me.lid);
                        msg.key = {
                            remoteJid,
                            fromMe: fromMeJid || fromMeLId,
                            participant: node.attrs.participant_pn  || node.attrs.sender_pn  || node.attrs.participant,
                            id: node.attrs.id,
                            ...(msg.key || {})
                        };
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if ((_b = msg.key.participant) === null || _b === void 0 ? void 0 : _b.endsWith('@lid')) {
                            msg.key.participant = node.attrs.participant_pn ? node.attrs.participant_pn : node.attrs.participant;
                        }
                        if ((0, WABinary_1.isJidGroup)(msg.key.remoteJid) && ((_f = (_e = (_d = (_c = msg.message) === null || _c === void 0 ? void 0 : _c.extendedTextMessage) === null || _d === void 0 ? void 0 : _d.contextInfo) === null || _e === void 0 ? void 0 : _e.participant) === null || _f === void 0 ? void 0 : _f.endsWith('@lid'))) {
                            if (msg.message.extendedTextMessage.contextInfo) {
                               ...
                               if (msg.key.participant?.endsWith('@lid') || (node.attrs.participant && node.attrs.participant.endsWith('@lid'))) {
                        ...
                        if (!(0, WABinary_1.isJidGroup)(msg.key.remoteJid) && (0, WABinary_1.isLidUser)(msg.key.remoteJid)) {
                            const realSender = node.attrs.sender_pn || node.attrs.from;
                               if (!msg.key.fromMe && realSender && realSender.endsWith('@s.whatsapp.net')) {
                                  msg.key.remoteJid = realSender;
                                }
                        }
                        let participant = msg.key.participant_pn || msg.key.participant;
