...
const DONATE_URL = 'https://www.instagram.com/midsoune_'; 
...
const tokenizeCode = (code, language = 'javascript') => {
    ...
    while ((match = LEXER_REGEX.exec(code)) !== null) {
        if (match[1]) {
            ...
            blocks.push({ highlightType: CodeHighlightType.DEFAULT, codeContent: match[6] || match[0] });
        }
    }
    return blocks;
};
...
case RichSubMessageType.TEXT:
    const shouldAddInlineEntity = true;
    const inlineEntity = [{
        key: 'MidSoune',
        metadata: {
            reference_id: 1,
            reference_url: DONATE_URL,
            reference_title: 'Visite Instagram Profile',
            reference_display_name: 'Visite',
            sources: [{
                source_type: 'THIRD_PARTY',
                source_display_name: 'Visite',
    ...
    return {
        view_model: {
            primitive: {
                ...
                // 🔥 هنا فين كاتزيد لون الخلفية والتكست:
                // واتساب كيستعمل هاد الستيل ف الـ Meta AI للون الأصفر الدافئ
                custom_attributes: {
                    theme: "YELLOW_WARNING", // أو "YELLOW" على حسب نسخة الـ Protobuf اللي عندك
                    background_color: "0xFFFFF9E6", // كود اللون Hex شفاف (ARBG) للأصفر
                    text_color: "0xFF000000" // لون التكست كحل باش يجي باين فوق الأصفر
                },
...
const prepareRichResponseMessage = (content) => {
    const { code, contentText, expressions, footerText, headerText, items, language, table, text, title } = content;
    let submessages = [];
    ...
    // ✅ تصحيح: حيدنا الـ Else باش يقبل العناصر كاملين مدموجين إيلا صيفطتيهم دقة وحدة
    if (code) {
        let lang = language || 'javascript'; // ✅ تصحيح: كياخد الـ language الممررة ديريكت
        submessages.push({
...
const botMetadataSignature = () => {
    ...
    // ✅ تصحيح: استعمال الـ webcrypto الآمن ف بيئة Node.js
    crypto_1.webcrypto.getRandomValues(signature);
    return signature;
};
...
const botMetadataCertificate = (length = 700) => {
    ...
    certificate[0] = 48;
    certificate[1] = 130;
    // ✅ تصحيح: استعمال الـ webcrypto الآمن هنا أيضاً
    crypto_1.webcrypto.getRandomValues(certificate.subarray(2));
    return certificate;
};
