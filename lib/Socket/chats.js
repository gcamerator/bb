...
const makeChatsSocket = (config) => {
    ...
    const onWhatsAppV2 = async (...jids) => {
        ...
        if (results) {
            return results.list.filter((a) => !!a.contact).map(({ contact, id, lid }) => ({ jid: id, exists: contact, lid: lid }));
        }
    };
    const onWhatsApp = async (...phoneNumbers) => {
    const usyncQuery = new WAUSync_1.USyncQuery().withContactProtocol().withLIDProtocol()
    const cleanNumbers = phoneNumbers.map(jid => {
        if (jid.endsWith('@lid')) return jid;
        return jid.split('@')[0].replace(/[^\d]/g, '') + '@s.whatsapp.net';
    });

    for (const jid of cleanNumbers) {
        const user = new WAUSync_1.USyncUser();
        if (jid.endsWith('@lid')) {
            user.withLid(jid).withId(jid);
        } else {
            user.withPhone(`+${jid.split('@')[0]}`);
        }
    ...
    if (results && results.list) {
        results.list.forEach((node, i) => {
            const isExist = (node.contact === true) || !!node.lid || !!node.status;
            let finalJid = (node.id === 'undefined' || !node.id) ? cleanNumbers[i] : node.id;
              if (finalJid && !finalJid.includes('@')) {
                finalJid = `${finalJid.split(':')[0]}@s.whatsapp.net`;
            }
            output.push({ jid: finalJid, exists: isExist, lid: node.lid || (finalJid.endsWith('@lid') ? finalJid.split('@')[0] : undefined)
            });
        });
    }
    return output;
};
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs = 250) => {
        var _a;
    ...
    const handlePresenceUpdate = ({ tag, attrs, content }) => {
        ...
        const currentPresence = presence?.lastKnownPresence || 'unavailable';
        const lastOnline = presence?.lastSeen || "Offline";
        if (presence) {
        ev.emit('presence.update', { chat: jid, user: participant,    presence: currentPresence, lastOnline  });
        } else {
        ev.emit('presence.update', { chat: jid, user: participant,    presence: 'unavailable', lastOnline });
       }
    };
