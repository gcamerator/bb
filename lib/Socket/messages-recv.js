...
const makeMessagesRecvSocket = (config) => {
    ...
    const terminateCall = async ( callId, callTo, callCreator = false,		reason  =null,		duration = 0	) => {
		const meId = authState.creds.me?.id
		...
		const stanza = {
			tag: 'call',
			attrs: { to: callTo, id: (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase() },
    ...
    const createCallLink = async (media = 'video', event, timeoutMs = 30000) => {
	...
	const joinCallLink = async (token, media = 'audio') => {
     if (!['audio'].includes(media)) {
        media = 'video';
    }
		const joinContent = [
    ...
    const handleReceipt = async (node) => {
        ...
        const sender = attrs.participant_pn || attrs.sender_pn || attrs.from;
       const isNodeFromMe = (authState.creds.me && (0, WABinary_1.areJidsSameUser)(sender, authState.creds.me))
    ...
    const handleNotification = async (node) => {
       ...
       // if("picture") type
        const remoteJid = node.attrs.participant_pn || node.attrs.sender_pn || node.attrs.from;
        if (shouldIgnoreJid(remoteJid) && remoteJid !== '@s.whatsapp.net') {
