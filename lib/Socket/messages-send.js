...
const makeMessagesSocket = (config) => {
    ...
    const getTypeMessage = (msg) => {
        ...
        else if (msg.reactionMessage || msg.encReactionMessage) {
            return 'reaction';
        }
        else if (msg.pollCreationMessage || msg.pollCreationMessageV5 || msg.pollCreationMessageV2 || msg.pollCreationMessageV3 || msg.pollUpdateMessage) {
            return 'poll';
        }
    ...
    const getMediaType = (message) => {
        ...
        else if (message.groupInviteMessage || message.extendedTextMessage?.matchedText) {
            return 'url';
        }
    };
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                let mediaHandle;
// Check if the content is a PTT (Voice Note) to ensure compatibility
if (typeof content === 'object' && content.audio && content.ptt) {
    const fs = require('fs');
    const path = require('path');
    const { execSync } = require('child_process');

    try {
        // Resolve input path from URL or Buffer
        let inputPath = content.audio.url || content.audio;
        
        // If audio is a Buffer, write it to a temporary file first
        if (Buffer.isBuffer(content.audio)) {
            inputPath = path.join(__dirname, `temp_in_${Date.now()}.mp3`);
            fs.writeFileSync(inputPath, content.audio);
        }

        const outputPath = path.join(__dirname, `temp_out_${Date.now()}.opus`);

        // Execute FFmpeg to convert audio to WhatsApp-compatible Opus format
        // -c:a libopus: Use Opus codec
        // -ac 1: Set audio channels to mono
        // -ar 48000: Set sample rate to 48kHz (standard for WA PTT)
        execSync(`ffmpeg -i "${inputPath}" -c:a libopus -ac 1 -ar 48000 -b:a 32k -application voip "${outputPath}" -y`);

        // Read the converted file back into a Buffer
        content.audio = fs.readFileSync(outputPath);
        content.mimetype = 'audio/ogg; codecs=opus'; 

        // Cleanup temporary files to save disk space
        if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath);
        if (typeof inputPath === 'string' && inputPath.includes('temp_in_')) {
            if (fs.existsSync(inputPath)) fs.unlinkSync(inputPath);
        }

        logger.info('Audio converted to PTT Opus successfully');
    } catch (err) {
        // Log error and fallback to original file if conversion fails
        logger.error({ err }, 'FFmpeg PTT conversion failed');
    }
}
                ...
                if (isDeleteMsg || isKeepMsg) {
                 ...
                 additionalNodes.push({
                        ...
                        attrs: {
                            ...
                            contenttype: isPollMsg ? 'text' : undefined
                        },
                        content: undefined
                    });
                 }
                else if (isAiMsg) {
                    console.log(fullMsg.message.messageContextInfo)
                    if ('messageContextInfo' in fullMsg.message && !!fullMsg.message.messageContextInfo) {
                        fullMsg.message.messageContextInfo.supportPayload = '{"version":1,"is_ai_message":true,"should_show_system_message":true,"ticket_id":"7004947587700716","citation_items":[],"ticket_locale":"us"}';
                    };
                    additionalNodes.push({
                        tag: 'bot',
                        attrs: {
            ...
            //    console.log(jid, fullMsg.message,fullMsg.key.id,options)
