...
const REAL_MSG_REQ_ME_STUB_TYPES = new Set([
    ...
    Types_1.WAMessageStubType.GROUP_PARTICIPANT_ADD_REQUEST_JOIN 
]);
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, keyStore, logger, options, getMessage }) => {
    ...
    if ((0, WABinary_1.isLidUser)(senderId)) {
        if (message.key.participant) {
            ...
            message.key.remoteJid = senderId;
        }
    }
    ...
    if (protocolMsg) {
        ...
        let participants;
        const actor = message.participant || message.key?.participant || message.key?.remoteJid;
        const emitParticipantsUpdate = (action) => (ev.emit('group-participants.update', { id: jid, author: actor, participants, action }));
        const emitGroupUpdate = (update) => {
            ev.emit('groups.update', [{ id: jid, ...update, author: actor }]);
        };
        const emitGroupRequestJoin = (participant, action, method) => {
            ev.emit('group.join-request', { id: jid, author: actor, participant, action, method });
        };
        const participantsIncludesMe = () => participants.find(jid => (0, WABinary_1.areJidsSameUser)(meId, jid));
        switch (message.messageStubType) {
            case Types_1.WAMessageStubType.GROUP_PARTICIPANT_CHANGE_NUMBER:
                participants = message.messageStubParameters || [];
                emitParticipantsUpdate('changenumber');
                break;
           ...
           case Types_1.WAMessageStubType.GROUP_PARTICIPANT_INVITE:
            ...
            case Types_1.WAMessageStubType.GROUP_PARTICIPANT_ADD_REQUEST_JOIN:
                ...
                emitParticipantsUpdate('reqjoin');
                break;
        ...
        let pollMsg;
        try {
            pollMsg = await getMessage(creationMsgKey);
        } catch (e) {
            logger?.debug({ e }, 'Failed to fetch poll message');
        }
        if (pollMsg) {
