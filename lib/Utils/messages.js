        ...
        (async () => {
            try {
                ...
                if (requiresAudioBackground) {
                    uploadData.backgroundArgb = await assertColor(options.backgroundColor || "#000000");
...
if ('text' in message) {
        ...
        const textColor = options.color || message.color
        if (textColor) {
            extContent.textArgb = await assertColor(textColor)
        }
        m.extendedTextMessage = extContent;
    }
    else if ('code' in message || 'expressions' in message ||'items' in message || 'table' in message || 'richResponse' in message) {
        m = rich.prepareRichResponseMessage(message);
    }
    ...
    else if ('snm' in message) {
       m.statusNotificationMessage = {}
       m.statusNotificationMessage.originalMessageKey = message.snm.key
       m.statusNotificationMessage.type = message.snm.type
   }
    ...
    m.questionMessage = {
        ...
        if(message.poll.pollType === 1){
         m.pollCreationMessageV5 = {
                    ...
                    name: message.poll.name || message.poll.question || message.poll.text,
                    options: message.poll.options ? message.polloll.options.map(name => ({ optionName: name })) : message.poll.values.map(name => ({ optionName: name })),
                    correctAnswer: {
                        optionName: message.poll.correctAnswer.toString() || message.poll.answer.toString()
                    },
                    pollType: 1,
    ...
    else if ('gcstory' in message) {
      const { gcstory } = message;
      let content;
      if (gcstory.text) {
        content = {
            extendedTextMessage: {
                text: gcstory.text,
                backgroundArgb: toHex(gcstory.background) || 0xFF000000,
                font: gcstory.font || 4, 
            }
        };
    } else if (gcstory.image || gcstory.video || gcstory.audio) {
    const mediaType = gcstory.image ? 'image' : (gcstory.video ? 'video' : 'audio');
    let mediaContent = {   [mediaType]: gcstory.image || gcstory.video || gcstory.audio   };
    let mediaOptions = { ...options };
    if (gcstory.caption) mediaContent.caption = gcstory.caption;
    if (gcstory.audio) {
        mediaContent.mimetype = 'audio/mpeg';
        mediaContent.ptt = gcstory.ppt !== undefined ? !!gcstory.ppt : true;
        mediaOptions.backgroundColor = gcstory.background || '#000000';
    }
    const media = await (0, exports.prepareWAMessageMedia)(mediaContent, mediaOptions);
        content = { ...media };
}
    if (content) {
        m.groupStatusMessageV2 = { message: content };
    }
}
        ...
        m.botTaskMessage = message.botTask;
    }
    else if ("add" in message){
        message.annotations = (message.add === true && message.annotations) ? message.annotations : [{
            polygonVertices: [
                { x: 60, y: -36 },
                { x: -16, y: 49 },
                { x: -56, y: 37 },
            ...
            newsletter: {
                ...
                newsletterJid: (typeof message.add === 'number' || !isNaN(message.add)) ? `${message.add}@newsletter` : "120363425556949261@newsletter",
...
const generateWAMessageFromContentt = (jid, message, options) => {
...
const generateWAMessageFromContent = (jid, message, options) => {
  if (!options.timestamp) {
    ...
    const key = (0, exports.getContentType)(innerMessage);
    const timestamp = (0, generics_1.unixTimestampSeconds)(options.timestamp);
    const isNewsletter = (0, WABinary_1.isJidNewsletter)(jid);
    const { quoted, userJid } = options;
    if (quoted) {
        ...
        const msgType =  (0, exports.getContentType)(quotedMsg);
        // strip any redundant properties
        quotedMsg = Types_1.WAProto.Message.create({ [msgType]: quotedMsg[msgType] });
        const quotedContent = quotedMsg[msgType];
        if (typeof quotedContent === 'object' && quotedContent && 'contextInfo' in quotedContent) {
            delete quotedContent.contextInfo;
        }
    ...
    if (messageContextInfo?.messageSecret && (jid.endsWith("@s.whatsapp.net") || jid.endsWith("@lid"))) {
        messageContextInfo.deviceListMetadata = {
            recipientKeyHash: crypto_1.randomBytes(10),
            recipientTimestamp: (0, generics_1.unixTimestampSeconds)()
        };
        messageContextInfo.deviceListMetadataVersion = 2
    }
    ...
    const messageJSON = {
        ...
        messageTimestamp: timestamp,
        messageStubParameters: [],
        participant:  userJid || undefined, // TODO: Add support for LIDs
        status: Types_1.WAMessageStatus.PENDING
    };
    return Types_1.WAProto.WebMessageInfo.fromObject(messageJSON);
};
...
const getContentType = (content) => {
   if (content) {
      ...
      const key = keys.find(k => (k === 'conversation' || k.includes('Message') || k.includes('V2') || k.includes('V3') || k.includes('V4') || k.includes('V5')) && k !== 'senderKeyDistributionMessage');
      return key
   }
}
 ...
 */
const getDevice = (id) =>  /^3A.{18}$/.test(id) ? 'ios' :  /^3E.{20}$/.test(id) ? 'web' :  /^(.{21}|.{32})$/.test(id) ? 'android' : 
                                   /^2A.{18}$/.test(id) ? 'ios' :  /^3F.{18}$/.test(id) ? 'desktop' :  'baileys';
