"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getPlatformId = exports.Browsers = void 0;

const os = require("os");
const { proto } = require("../../WAProto/index.js"); 

const PLATFORM_MAP = {
    'aix': 'AIX',
    'darwin': 'Mac OS',
    'win32': 'Windows',
    'android': 'Android',
    'freebsd': 'FreeBSD',
    'openbsd': 'OpenBSD',
    'sunos': 'Solaris',
    'linux': 'Ubuntu',  // الافتراضي لنظام لينكس
    'haiku': undefined,
    'cygwin': undefined,
    'netbsd': undefined
};

// تم تصحيح exports.Browsers هنا
// 1. تعريف الدالة الأساسية التي تستدعيها المكتبة افتراضياً
const Browsers = (browser = 'Chrome') => {
    return [
        PLATFORM_MAP[os.platform()] || 'Ubuntu',
        browser,
        os.release()
    ];
};

// 2. إضافة الدوال الأخرى كخصائص لهذه الدالة لضمان التوافق
Browsers.ubuntu = (browser = 'Chrome') => ['Ubuntu', browser, '22.04.4'];
Browsers.macOS = (browser = 'Chrome') => ['Mac OS', browser, '14.4.1'];
Browsers.windows = (browser = 'Chrome') => ['Windows', browser, '10.0.22631'];
Browsers.baileys = (browser = 'Baileys') => ['Baileys', browser, '6.5.0'];
Browsers.android = (version = '15') => ['Android', 'android', version];
Browsers.and_roid = (version = '15') => ['Android', 'and_roid', version];
Browsers.appropriate = Browsers; // الدالة نفسها تعمل كـ appropriate

// 3. تصدير الدالة
exports.Browsers = Browsers;

const getPlatformId = (browser) => {
    const key = browser.toUpperCase();

    // التحقق من منصة أندرويد
    if (key === 'ANDROID' || key === 'AND_ROID') {
        return proto.DeviceProps.PlatformType.ANDROID_PHONE.toString();
    }

    // محاولة جلب النوع من ملف الـ proto أو العودة للمتصفح الافتراضي
    return (
        proto.DeviceProps.PlatformType[key]?.toString() ||
        proto.DeviceProps.PlatformType.CHROME.toString()
    );
};

// تصدير الدالة
exports.getPlatformId = getPlatformId;
